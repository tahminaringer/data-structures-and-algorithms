...
const templateWithJQuery = () => {
  starWarsPeople.forEach(person => {
    ...
    let $getNew = $(`<section>${getTemplate}</section>`);
    $getNew.find('h2').text(person.name);
    $getNew.find('h3').text(person.height);
    $getNew.find('p').text(person.eye_color);
    $('main').append($getNew);
  });
};
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(word => {
    let oneSlice = word.slice(word.indexOf(' ') + 1);
    let twoSlice = oneSlice.slice(oneSlice.indexOf(' ') + 1);
    result.push(twoSlice);
  });
  return result;
};
