...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.status(200).send('I am am Tahmina, I am originally from Montana and I am a fullstack javascript developer with a background in floral and event design.');
  });
  const favoriteFoods = ['home grown anything', 'salads', 'ny style pizza', 'fruit'];
  app.get('/favoritefoods', (request, response) => {
    response.status(200).send(favoriteFoods);
  });
...
const count = (target, input) => {
  const newNumber = input.reduce((accumulator, num) => {
    num.forEach(number => {
      if (number === target) {
        accumulator ++;
      }
    ...
    return accumulator;
    // num?num === target: counter ++; turinary function
  }, 0);
  return newNumber;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const newArray = [];
  input.forEach(num => {
    const divisibleByFive = num.filter(numFive => numFive % 5 === 0 && typeof(numFive) === 'number');
    const powerArr = divisibleByFive.map(numSquared => {
      return Math.pow(2, numSquared);
    });
    newArray.push(powerArr);
  });
  return newArray;
};
